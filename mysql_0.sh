...
# Build File Name
FILE_NAME=$DATE_FORMAT-$DB_NAME_0".sql.gz"

# Print message
echo "Beginning MySQL Dump"

# SSH into the server, complete a mysql dump and download the file
ssh $REMOTE_HOST -p $REMOTE_PORT "mysqldump --no-tablespaces --databases $DB_NAME_0 --host=$DB_HOST --user=$DB_USER --password=$DB_PASSWORD | gzip -9"  > $BACKUPS/$BACKUPS_MYSQL/$FILE_NAME
