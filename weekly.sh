...
if [ ! -z "$DB_NAME_0" ] 
then 
  WEEKLY_MYSQL_0=$(ls backups/mysql -t | egrep $DB_NAME_0'\.sql.gz' | head -1)
  [ ! -z "$WEEKLY_MYSQL_0" ] && cp $BACKUPS/$BACKUPS_MYSQL/$WEEKLY_MYSQL_0 $BACKUPS/$BACKUPS_WEEKLY/$WEEKLY_MYSQL_0
fi

if [ ! -z "$DB_NAME_1" ] 
then 
  WEEKLY_MYSQL_1=$(ls backups/mysql -t | egrep $DB_NAME_1'\.sql.gz' | head -1)
  [ ! -z "$WEEKLY_MYSQL_1" ] && cp $BACKUPS/$BACKUPS_MYSQL/$WEEKLY_MYSQL_1 $BACKUPS/$BACKUPS_WEEKLY/$WEEKLY_MYSQL_1
fi

if [ ! -z "$DB_NAME_2" ] 
then 
  WEEKLY_MYSQL_2=$(ls backups/mysql -t | egrep $DB_NAME_2'\.sql.gz' | head -1)
